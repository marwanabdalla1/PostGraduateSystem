...
as
If(exists(
select* from defense where serialNumber=@ThesisSerialNo and date=@DefenseDate))
begin
...
from NonGucianStudentTakeCourse NC inner join Course C 
on (NC.cid = C.id)
where sid = @studentID
go
...
@title varchar(50), @pubDate datetime, @host varchar(50), @place varchar(50), @accepted bit, @output bit output, @pubID int output
as
...
select T.* From GucianStudent G inner join GUCianStudentRegisterThesis GT
on (G.id=GT.sid) inner join Thesis T on (GT.serial_no = T.serialNumber)
where G.id = @ID
if(exists(select * from NonGucianStudent where id=@ID))
select T.* From NonGucianStudent N inner join NonGUCianStudentRegisterThesis NT
on (N.id=NT.sid) inner join Thesis T on (NT.serial_no = T.serialNumber) 
where N.id = @ID
...
GO
create proc MaxReportNoTGucian
@id int,
@result INT OUTPUT
AS
if exists (select * from NonGUCianProgressReport where sid = @id)
begin 
SELECT @result = max (no)
from NonGUCianProgressReport GP
where sid = @id
end 
else begin 
set @result=0
...
create proc ThesisEndDateValid
@Tid int,
@accepted bit output
As 
if exists (select * from Thesis where serialNumber = @Tid)
	begin 
	declare @endDate datetime
	SELECT @endDate = endDate
	from Thesis T
	where serialNumber = @Tid
...
@supervisorID int
as
select * from GUCianProgressReport g where exists(select * from GUCianStudentRegisterThesis t where g.sid=t.sid and @supervisorID=t.supid)
union (select * from NonGUCianProgressReport g where exists (select * from NonGUCianStudentRegisterThesis t where g.sid=t.sid and @supervisorID=t.supid))

go
create proc CheckGUCianThesis
@serialNo int,
@GUCian bit output
as
if exists(select * from GUCianStudentRegisterThesis g where g.serial_no = @serialNo)
begin
set @GUCian='1'
end
else
...
as
update Examiner 
set name = @name, fieldOfWork=@fieldOfWork, isNational=@isNational
where id =@id
...
create proc viewexaminerInfoDefense
@id int
As

select g.firstName,g.lastName,sup.name,t.title
from ExaminerEvaluateDefense ee 
inner join examiner e on ee.examinerId=e.id
inner join GUCianStudentRegisterThesis s on ee.serialNo=s.serial_no
inner join Supervisor sup on sup.id=s.supid
inner join GucianStudent g on g.id = s.sid
inner join thesis t on t.serialNumber=s.serial_no
where @id = e.id
union
select Ng.firstName,Ng.lastName,supp.name,ts.title
from ExaminerEvaluateDefense ee 
inner join examiner e on ee.examinerId=e.id
inner join NonGUCianStudentRegisterThesis sg on ee.serialNo=sg.serial_no
inner join Supervisor supp on supp.id=sg.supid
inner join NonGucianStudent Ng on Ng.id = sg.sid
inner join thesis ts on ts.serialNumber=sg.serial_no
where @id = e.id
...
@keyword  Varchar(100),@success bit Output
as
if(exists(
Select* from Thesis
...
@thesisSerialNo int, @progressReportDate date, @studentID int,@progressReportNo int,  @output int output
as
declare @gucian int
if(exists(
select id
...
if(@gucian = '1')
	...
	if exists(select * from GUCianStudentRegisterThesis T where @studentID = T.sid and @thesisSerialNo= T.serial_no )
			begin
			declare @AB bit 
			execute ThesisEndDateValid @thesisSerialNo, @AB output
			if ( @AB =1)
				begin
				declare @maxG int
						execute MaxReportNoGucian @studentID, @maxG output
						 set @progressReportNo = @maxG+1 
	...
	else if not exists(select * from GUCianStudentRegisterThesis T where @studentID = T.sid and @thesisSerialNo= T.serial_no )
			begin 
			set @output = 0
			end
			end
	...
	if exists(select * from NonGUCianStudentRegisterThesis T where @studentID = T.sid and @thesisSerialNo= T.serial_no)
	begin
			declare @ABN bit 
			execute ThesisEndDateValid @thesisSerialNo, @ABN output
			if ( @ABN =1)
				begin
				declare @max int
				execute MaxReportNoTGucian @studentID, @max output
				 set @progressReportNo = @max+1 
		...
		else if not exists(select * from NonGUCianStudentRegisterThesis T where @studentID = T.sid and @thesisSerialNo= T.serial_no)
			begin 
			set @output = 0
			end
	end 
...
varchar(200),@studentID int,  @output bit output
as
declare @gucian bit
if(exists(
...
if(@gucian = '1')
	begin
	if exists (select * from GUCianProgressReport NP where thesisSerialNumber = @thesisSerialNo and sid = @studentID and no =
	@progressReportNo)
		begin 
		declare @ABN bit 
			execute ThesisEndDateValid @thesisSerialNo, @ABN output
...
begin
if exists (select * from NonGUCianProgressReport NP where thesisSerialNumber = @thesisSerialNo and sid = @studentID and no =
@progressReportNo) 
	begin
	declare @ABNN bit 
			execute ThesisEndDateValid @thesisSerialNo, @ABN output
...
@PubID int, @thesisSerialNo int, @studentID INT, @output int output 
as
IF exists(select * from GUCianStudentRegisterThesis T where @studentID = T.sid and @thesisSerialNo= T.serial_no 
UNION select * from NonGUCianStudentRegisterThesis g where @studentID = g.sid and @thesisSerialNo= g.serial_no  )
			BEGIN
--check law el thesis end date 3ada el awel
							declare @ABN bit 
							execute ThesisEndDateValid @thesisSerialNo, @ABN output
							...
							if ( @ABN =1)
								...
								begin 
									set @output = -1  /*thesis expired */
								end
				END
			else 
				begin 
				set @output = 0  /* Thesis msh beta3to */
				end
